...
function tokcall($no, $jum, $wait){
    ...
    while($x < $jum) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,"https://store.steampowered.com/phone/add_ajaxop");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS,"op=get_phone_number&input=%2B62+".$no."&sessionID=72f30887c4c3530c9c0605ac&confirmed=1&checkfortos=1&bisediting=0&token=0");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_REFERER, 'https://store.steampowered.com/phone/add');
